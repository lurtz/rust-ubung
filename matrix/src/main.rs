...
impl<T: Clone + Default> Matrix<T> {
  fn new(rows: u32, cols: u32) -> Matrix<T> {
    let init = T::default();
    Matrix{rows: rows, cols: cols, data: vec![init; (rows*cols) as usize]}
  }
}

impl<T> Matrix<T> {
  fn set(& mut self, row: u32, col: u32, val: T) {
...
impl<T: Clone + Mul<Output=T> + Add<Output=T>> Mul for Matrix<T> {
  ...
  fn mul(self, _rhs: Matrix<T>) -> Matrix<T> {
    ...
    Matrix{rows: self.rows, cols: _rhs.cols, data: data}
  }
}
...
//    assert_eq![0, m.cols];
  ...
  #[test]
  fn mult_4x4_matrix() {
    assert_eq!(mmatrix![19, 22; 43, 50], mmatrix!(1,2;3,4) * mmatrix!(5,6;7,8));
  }
